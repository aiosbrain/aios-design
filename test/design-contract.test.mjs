...
test("package, lockfile, and published contract identify the same patch", () => {
  ...
  assert.match(readme, /@aios-alpha\/design@\^0\.6\.0 @aios-alpha\/ui@\^0\.6\.0/);
  assert.match(readme, /0\.6\.0 does not change token values/);
...
test("release tag verification fails closed and accepts only the package version", () => {
  ...
  assert.notEqual(run("tag", "v1.0.0").status, 0);
  assert.equal(run("tag", "v0.6.0").status, 0);
});
