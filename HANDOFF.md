...
**Status (2026-06-22): packages PUBLISHED, all phases have PRs.** `@aios-alpha/design@0.1.0` + `@aios-alpha/ui@0.1.0` are live on npm. This repo is now `AIOS-alpha/aios-design` on GitHub (public, `main`). **P1/website ✅ MERGED** (#4). **P2/team-brain** PR [#68](https://github.com/AIOS-alpha/aios-team-brain/pull/68). **P3/workspace-GUI** PR [#86](https://github.com/AIOS-alpha/aios-workspace/pull/86). **P4** = website Design System doc PR [#5](https://github.com/AIOS-alpha/aios-alpha.github.io/pull/5) + root CLAUDE.md + docs-sync (scaffold itself ships no frontend → the GUI is the inheritance path). All verified locally; pending user review/merge. Trusted Publishing wired (`.github/workflows/publish.yml`) — needs the per-package npmjs.com web-UI link to go live. Backlog epic: **W2.5** (Linear AIO-79..83, all In Review/Done). Memory: `project-design-system-rollout`.
